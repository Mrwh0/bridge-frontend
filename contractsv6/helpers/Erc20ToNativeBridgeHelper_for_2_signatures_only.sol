...
contract Erc20ToNativeBridgeHelper is Helper {
    ...
    function getSignatures(bytes32 _msgHash) public view returns(bytes memory) {
        ...
        // !!!WARNING!!!
        // This particular helper works only with 3 signatures collected on the Home bridge
        // If the configuration of the bridge is changed, another helper must be prepared
        require(signed == 2, "2 confirmations expected");
        
        bytes32 r0; bytes32 r1; 
        bytes32 s0; bytes32 s1; 
        bytes1 v0;  bytes1 v1;  
        ...
        return(abi.encodePacked(uint8(2), v0, v1, r0, r1, s0, s1));
    }
